...
namespace University.Blazor.Application.Services
{
    public class IdentityService : IIdentityService
    {
        ...
        public IdentityService(IHttpClientFactory factory, ISyncLocalStorageService syncLocalStorageService, AuthenticationStateProvider authStateProvider)
        {
            this.httpClient = factory.CreateClient("University");
        ...
        public async Task<bool> Login(string userName, string password)
        {
            ...
            if (!string.IsNullOrEmpty(response.UserToken)) // login success
            {
                ...
                syncLocalStorageService.SetUsername(response.Email);

                ((AuthStateProvider)authStateProvider).NotifyUserLogin(response.Email);
